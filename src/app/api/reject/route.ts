...
import { emitCancelUpdate, emitRejectUpdate } from "@/lib/sseEmitter";
    ...
    if (actionRequestId) {
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (request.requestedBy === session.user.id) {
          return NextResponse.json({ error: 'You cannot reject your own requests' }, { status: 403 });
        }
        ...
        await logActivity(tx, {
          action: request.action === "PAUSE" ? ActivityAction.SYSTEM_PAUSE_REQUEST_REJECT : ActivityAction.SYSTEM_RESUME_REQUEST_REJECT,
          metadata: {
            actionRequestId,
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (request.requestedBy === session.user.id) {
          return NextResponse.json({ error: 'You cannot reject your own requests' }, { status: 401 });
        }
        ...
        await logActivity(tx, {
          action: updated?.action === 'UNFREEZE' ? ActivityAction.UNFREEZE_REQUEST_REJECT : ActivityAction.FREEZE_REQUEST_REJECT,
          metadata: {
            freezeRequestId,
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        await logActivity(tx, {
          action: updated?.action === 'UNBLACKLIST' ? ActivityAction.UNBLACKLIST_REQUEST_REJECT : ActivityAction.BLACKLIST_REQUEST_REJECT,
          metadata: {
            blacklistRequestId,
            address: request.address,
            request: JSON.stringify(updated, (key, value) =>
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        await logActivity(tx, {
          action: ActivityAction.REFUND_REQUEST_REJECT,
          metadata: {
            refundRequestId,
            request: JSON.stringify(updated, (key, value) =>
              typeof value === 'bigint' ? value.toString() : value
