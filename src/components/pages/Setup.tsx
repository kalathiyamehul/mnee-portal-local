...
const Setup = () => {
  ...
  const validateTokenId = useCallback((tokenId: string) => {
    // Regex pattern for txid_vout (64 hex chars, underscore, digits)
  ...
  const getTokenDetails = useCallback(
    async (tokenId: string) => {
      ...
      try {
        // Fetch token details using tokenId
        const res = await fetch(`/api/token/${tokenId}`);
        if (res.ok) {
          const tokenDetails: BSV21 = await res.json();
          // Extract decimals from tokenDetails
          setDecimals(tokenDetails.dec);
          setTokenError("");
        } else {
  ...
  const handleAddFee = () => {
    if (newFee.min >= 0 && newFee.max >= newFee.min && newFee.fee >= 0) {
      ...
      // Handle invalid fee configuration
  ...
  const saveConfig = async (updatedFees: FeeConfig[] = fees) => {
    const res = await fetch("/api/config", {
      ...
      body: JSON.stringify({ tokenId, feeAddress, fees: updatedFees, decimals }),
    });
  ...
  // Function to clear the Config
  const clearConfig = async () => {
    const confirmClear = confirm("Are you sure you want to reset the configuration?");
    ...
    if (res.ok) {
      ...
      setDecimals(null);
      setTokenError("");
      router.push("/setup"); // Redirect to setup page if needed
    } else {
      // Handle error
      console.error("Error clearing configuration.");
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await saveConfig();
    router.push("/"); // Redirect after successful setup
  };
